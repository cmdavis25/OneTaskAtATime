...
**Status:** Planned feature, not implemented in v1.0

**Description:**
You cannot create tasks that automatically recreate (e.g., "Pay rent" every month).

**Workaround:**
- Manually create task each time
- Set calendar reminder to create task in OneTaskAtATime
- Use external scheduler (Windows Task Scheduler) to trigger reminder

**Planned Fix:** Recurring task support in v1.1.0
...
### Focus Mode Doesn't Remember Zoom Level

**Status:** Minor bug

**Description:**
If you use Ctrl+Mouse Wheel to zoom in Focus Mode, the zoom level resets when switching tasks.

**Workaround:**
- Use Settings → Font Size for persistent text scaling
- Zoom applies to current task only

**Planned Fix:** v1.1.0

---

### Dependency Graph Cluttered with Many Dependencies

**Status:** UI limitation

**Description:**
If a task has 20+ dependencies, the graph view becomes difficult to read due to overlapping nodes and edges.

**Workaround:**
- Use hierarchical layout (Settings → Graph → Layout: Hierarchical)
- Export graph as image and zoom in external viewer
- Review dependencies in Task List (Dependency column)

**Planned Fix:** Improved graph layout algorithms in v1.3.0

---

### Theme Switch Requires Restart for Some Dialogs

**Status:** Minor bug

**Description:**
When switching themes, some dialogs (Help, Settings, Shortcuts) may not update until application restart.

**Workaround:**
- Close dialog and reopen after theme change
