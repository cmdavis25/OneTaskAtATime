...
class TaskDAO:
    ...
    def create(self, task: Task) -> Task:
        ...
        cursor.execute(
            """
            INSERT INTO tasks (
                title, description, base_priority, priority_adjustment,
    ...
    def get_by_id(self, task_id: int) -> Optional[Task]:
        ...
        cursor.execute(
            """
            SELECT id, title, description, base_priority, priority_adjustment,
                   due_date, state, start_date, delegated_to, follow_up_date,
                   completed_at, context_id, last_resurfaced_at, resurface_count,
                   created_at, updated_at
            FROM tasks
    ...
    def update(self, task: Task) -> Task:
        ...
        cursor.execute(
            ...
            UPDATE tasks SET
                ...
                priority_adjustment = ?, due_date = ?, state = ?,
    ...
    def _row_to_task(self, row: sqlite3.Row) -> Task:
        ...
        return Task(
            ...
            start_date=date.fromisoformat(row[7]) if row[7] else None,
            delegated_to=row[8],
            follow_up_date=date.fromisoformat(row[9]) if row[9] else None,
            completed_at=datetime.fromisoformat(row[10]) if row[10] else None,
            context_id=row[11],
            last_resurfaced_at=datetime.fromisoformat(row[12]) if row[12] else None,
            resurface_count=row[13],
